...
from dragonsr.Config import (
    Experiment as _CfgExp,
    Paths as _CfgPaths,
    PySR as _CfgPySR,
    DRAGON_METHODS as DRAGON_METHOD_CONFIGS,
)
from dragonsr.dataprocessing.Data import DatasetLoader
from dragonsr.runner.Dragon import run_dragon_method
from dragonsr.helpers.html_builder import build_html
from dragonsr.helpers.Helper import apply_text_config
from pathlib import Path


def _get_run_pysr():
    from dragonsr.runner.pysr_runner import run_pysr
    return run_pysr
...
def cli(argv=None):
    """Command-line entry point (``python -m dragonsr`` / ``dragonsr``)."""
    import argparse
    mp.set_start_method("spawn", force=True)
    parser = argparse.ArgumentParser(prog="dragonsr", description="DragonSR / PySR symbolic-regression benchmark pipeline")
