...
This script runs one of:
    python -m dragonsr --run_dragonsr --config_file_path ... [--data_path ...]
    python -m dragonsr --pysr-only   --config_file_path ... [--data_path ...]
...
def _resolve_relative(raw: str, plan_file: Path) -> Path:
    """Resolve a path from a plan file: absolute as is, otherwise relative to the plan file's folder."""
    p = Path(raw).expanduser()
    if not p.is_absolute():
        candidate = (plan_file.parent / p)
        if candidate.exists():
            ...
            runs.append(
                {
                    ...
                    "config": str(_resolve_relative(cfg, plan_file)),
                    "data_path": (str(_resolve_relative(data_path, plan_file)) if data_path else None),
                }
            )
    return runs
...
def _run_one(
    *,
    ...
    cmd.insert(3, "--run_dragonsr" if engine == "dragonsr" else "--pysr-only")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--work-dir",
        default=".",
        help="Working directory where leaderboard_runs/ is created (default: current directory).",
    )
    parser.add_argument(
        "--html-archive-dir",
...
def main() -> int:
    ...
    main_script = Path("python -m dragonsr")  # kept for log/manifest compatibility; the module is invoked directly
